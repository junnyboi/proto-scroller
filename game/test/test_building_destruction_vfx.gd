...
func test_section_burst_pool_is_fixed_and_recycles_oldest_slot() -> void:
	...
	assert_eq(third.fragments.amount, 8)
	assert_eq(third.falling_debris.amount, 7)
	assert_eq(third.dust.amount, 8)
...
func test_material_profiles_select_distinct_generated_fragment_textures() -> void:
	...
	assert_eq(concrete.fragments.amount, 12)
	assert_eq(glass.fragments.amount, 18)
	assert_eq(steel.fragments.amount, 8)
	assert_eq(concrete.falling_debris.amount, 10)
	assert_eq(glass.falling_debris.amount, 13)
	assert_eq(steel.falling_debris.amount, 7)
	assert_eq(concrete.dust.amount, 14)
	assert_eq(glass.dust.amount, 7)
	assert_eq(steel.dust.amount, 8)
