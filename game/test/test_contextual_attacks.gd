...
func test_resolver_locks_ground_at_699_and_jab_cross_at_700() -> void:
	...
	assert_almost_eq(AttackResolver.GROUND_SMASH_DAMAGE_MULTIPLIER, 2.0, 0.0001)
	assert_almost_eq(ground.actor_damage, 360.0, 0.001)
	assert_almost_eq(ground.structural_damage, 360.0, 0.001)
...
func test_tap_charge_releases_normal_damage_and_full_charge_caps_at_double_damage() -> void:
	...
	assert_almost_eq(attacks.current_spec.structural_damage, 720.0, 0.001)
...
func test_full_charge_ground_commit_emits_double_damage_with_base_impulse_and_radius() -> void:
	...
	assert_almost_eq(float(payloads[0].actor_damage), 720.0, 0.001)
	assert_almost_eq(float(payloads[0].structural_damage), 720.0, 0.001)
