...
The expanded Simplified Chinese narrative catalog currently requires 929 unique catalog codepoints after the title command hook and district-shop vocabulary are synchronized. `scripts/build-cjk-font-subset.py` deterministically rebuilds `game/art/fonts/DroidSansFallbackFull-ProtoScroller.ttf` from the Apache-2.0 system source `/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf`, retaining only current `zh-CN.json` characters plus invariant control glyphs. The current subset is 139,580 bytes with SHA-256 `3f758e8cf236fdbe02cbfc2070f547a20736859f6713d2f16084c80e46e177bf` and passes the repository's full-catalog glyph coverage test.
