...
The expanded Simplified Chinese narrative catalog currently requires 928 unique catalog codepoints after the title command hook and district-shop vocabulary are synchronized. `scripts/build-cjk-font-subset.py` deterministically rebuilds `game/art/fonts/DroidSansFallbackFull-ProtoScroller.ttf` from the Apache-2.0 system source `/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf`, retaining only current `zh-CN.json` characters plus invariant control glyphs. The current subset is 139,452 bytes with SHA-256 `228e141419d2eb2afcdc30b50e2e94586fcce73aec56b6521a4d395ba6f76023` and passes the repository's full-catalog glyph coverage test.
