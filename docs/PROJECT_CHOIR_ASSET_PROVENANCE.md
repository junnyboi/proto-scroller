...
The expanded Simplified Chinese narrative, tutorial, upgrade, and runtime-tuning catalog currently requires 996 unique catalog codepoints after the title, district-shop, 56-parameter tuning, visual-control, new-skill, and tuned-run debrief vocabulary are synchronized. `scripts/build-cjk-font-subset.py` deterministically rebuilds `game/art/fonts/DroidSansFallbackFull-ProtoScroller.ttf` from the Apache-2.0 system source `/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf`, retaining only current `zh-CN.json` characters plus invariant control glyphs. The current subset is 149,636 bytes with SHA-256 `ce637969709052c8d8006bbf840d68e62cffe7c30eda0846b2a4bb887bbe025e` and passes the repository's full-catalog glyph coverage test.
