...
Every building retains a **3×2 logical structural grid** so the player can read targets at speed. Each damaged cell progressively erodes its one authored facade sprite from the center outward: a small initial void grows through a stable jagged boundary while all surviving facade pixels darken with cumulative damage. A living damaged section owns exactly one deterministic accent—video-derived fire, one broken pipe, or one dangling cable—plus its cracks. Terminal failure removes most of the center and lower middle, leaves dark irregular side rails and a top lintel around the opening, and culls every fire/wire/pipe accent. Transparent source pixels remain transparent. **Every terminal cell leaves a shallow foreground pile made only from transparent concrete, glass, or steel fragments at that cell’s lower edge. The pile preserves material readability, then blends in a restrained district grade: cold finance cyan for Business, weathered municipal teal for Residential, muted neon magenta for Entertainment, field-olive amber for Military, and ceremonial bronze-gold for Royal. Its first live activation emits one short, low-opacity dust cloud from the existing twelve-slot destruction pool; stream restore never replays it. No destroyed cross-section sprite, replacement facade, stretched rubble backdrop, interior plate, or other background is ever added behind a failed cell; procedural erosion remains the sole persistent facade rendering.** Lower-cell failure transfers damage upward; complete floors start staggered collapse; loss of all authored steel supports triggers the faster steel-support chain.[1]

The same terminal-remains policy and current district grade apply to streamed cars and streetlamps and to destructible catalyst structures. Their authored broken or spent sprites are permitted only during the nonterminal wreck/spent stage. Full destruction hides those sprites, disables their collision, leaves only a fixed, nonblocking rubble-fragment bed, and requests the same subtle dust-only pool activation exactly once. Boss defeats retain their existing authored rubble presentation. Temporary encounter hazards and enemy remains are separate pooled effect/combat systems rather than persistent destructible structures.
...
Each of the 25 buildings receives one standalone GPT Image 2 transparent facade sprite. One per-cell shader operates directly on that facade, preserving pristine alpha at zero damage and expanding a seed-stable jagged alpha void as normalized damage rises. Cracks, accent choice, and hollow cutouts remain runtime-generated and deterministic rather than requiring bespoke stage or cross-section images. The fire choice uses one shared 24-frame transparent WebP atlas generated through video-to-sprites; cables and pipes retain their compact generated sprites. Structural variants expose no damaged, rubble, cross-section, or interior-background texture channel. Persistent rubble beds reuse only the three transparent material-fragment textures already used by physical debris; they never sample a facade or cover a cell-sized rectangle.
