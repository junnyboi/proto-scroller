...
Body death starts a dedicated wreck generation that clears boss attacks, support utilities, and reservations while preserving the visible atlas rig. The hidden generic tank wreck remains the pooled scrap authority, but its sprite and all damage receivers are suppressed; the player sees the actual boss frozen on the last attack frame and darkened with the same defeated tint as ordinary wrecks for the fixed 2.95-second spectacle.

The spectacle's completion signal automatically scraps the hidden wreck exactly once. Generation cleanup then hides the frozen rig, reveals one prewarmed `RUBBLE_BED` presentation grounded on the road divider, and retains the existing bounded debris burst. The first two campaign bosses drop two repair cells; MIMESIS, CANTOR, and CHOIR Prime drop three. The existing catalyst runtime owns a fixed five-slot repair pool—two power-box slots plus three boss-drop headroom—and singular-boss reward scaling raises every cell to 150 chassis HP. Royal leaves its former outcome receivers inactive: a complete eligibility snapshot commits DISENTANGLE automatically, while every incomplete state safely commits PURGE before rubble conversion.
...
| WP6 — Defeated pose, rubble, and repairs | Complete; revised 2026-08-28 | Automatic handoff supersedes source `430f4d0` | The frozen, darkened final atlas frame remains through the full 2.95-second defeat spectacle; its completion signal automatically creates prewarmed road-grounded rubble and releases two or three fixed 150-HP pickups from the shared pool. |
