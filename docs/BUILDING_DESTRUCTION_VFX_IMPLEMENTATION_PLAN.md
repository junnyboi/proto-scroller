...
The generated concrete, glass, steel, dust, and flash assets remain active only as transient section bursts and physical macro debris. Persistent failed facade cells instantiate no `BuildingRubbleEdge2D`, rubble sprite, second facade shell, interior plate, or destroyed cross-section image. `BuildingDamagePattern2D` records fatal geometry before the stage transition for deterministic persistence, then both the authored facade sprite and procedural overlay become invisible at terminal failure. Damaged but living sections deterministically show only one of generated fire, one broken pipe, or one dangling cable; terminal failure stops and hides that accent immediately. Structural variants expose exactly one authored facade texture, so there is no secondary background channel for later work to re-enable. This contract applies automatically to all 25 district variants through the existing pooled reconfiguration path.
