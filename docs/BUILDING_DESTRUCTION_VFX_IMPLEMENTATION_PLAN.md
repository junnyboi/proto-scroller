...
The generated concrete, glass, steel, dust, and flash assets remain active only as transient section bursts and physical macro debris. Persistent failed facade cells instantiate no `BuildingRubbleEdge2D`, rubble sprite, second facade shell, interior plate, or destroyed cross-section image. Instead, `BuildingDamagePattern2D` preserves the single authored facade cell, applies its deterministic fatal geometry as a bottom-connected jagged arch, and leaves the remaining side piers and crown slightly darkened. Damaged but living sections deterministically show only one of generated fire, one broken pipe, or one dangling cable; terminal failure stops and hides that accent immediately. Structural variants expose exactly one authored facade texture, so the terminal ruin adds no secondary background channel or allocation. This contract applies automatically to all 25 district variants through the existing pooled reconfiguration path.
