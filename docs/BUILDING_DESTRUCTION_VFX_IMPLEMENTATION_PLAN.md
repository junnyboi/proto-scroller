...
The generated concrete, glass, steel, dust, and flash assets remain the source for transient section bursts, physical macro debris, and four fixed ground-rubble sprites beneath each failed road-contact bay. Persistent failed cells still instantiate no `BuildingRubbleEdge2D`, second facade shell, interior plate, stretched rubble backdrop, or destroyed cross-section image. `BuildingDamagePattern2D` preserves the single authored facade cell, maps its existing pattern seed to one of six bottom-connected jagged arch families, darkens the surviving side piers and crown more heavily, and applies a matching chipped upper silhouette to ground bays. The families vary coarse crown frequency, directional shear, serration, stepped breaks, and twin notches without adding media, saved fields, or runtime allocations. Ground bays reuse one fixed `PersistentRubbleBed2D` component with four material-matched fragments, deterministic concrete, steel, and glass scale ranges, and a brighter tint that remains legible over dark asphalt. The Residential Bluewire Laundry Walkup source also has true alpha behind its retained stair rails instead of an opaque interior field. Damaged but living sections deterministically show only one of generated fire, one broken pipe, or one dangling cable; terminal failure stops and hides that accent immediately. Every fresh failure also restarts a subtle residual-smoke emitter inside the existing twelve-slot burst pool. This contract applies automatically to all 25 district variants through the existing pooled reconfiguration path.
