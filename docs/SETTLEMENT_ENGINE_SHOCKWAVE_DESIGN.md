...
| **Release** | 1.05 s | The incoming motes, core sphere, and charge cue stop at one state boundary. One cyan-white circular shockwave expands from the same core to a 900-pixel radius, followed by four progressively dimmer blue wake echoes, accompanied by a one-second pressure cue and one restrained 10-unit camera impulse. | Only contact with the bright moving leading band inflicts damage; all wake echoes are cosmetic. |
...
The charge reuses the exact player photon source assets rather than introducing another visual dialect or increasing the Web package budget. `photon_core_orb.png` supplies both the converging motes and the energy sphere. `photon_release_shockwave.png` supplies the expanding release surface and four normal-blended earlier-radius wake samples. An additive alpha-and-luminance shader removes the core source texture's green bias while retaining internal filaments, so both core and motes read as saturated cyan/electric blue. S-04 scales the core far beyond the player's chest orb, surrounds it with the same warm target/countdown language as ordinary enemy attacks, and keeps all movement code-owned. The wake is intentionally thinner and lower-alpha than the bright leading band, preserving a readable safe interior and preventing cosmetic history from masquerading as damage.[2] [4]
...
`BossAttackArea2D` prewarms one `CPUParticles2D`, one energy-core sprite, one leading release sprite, four wake sprites, and two positional audio players only for its radial role. The existing fixed radial utility slot is reused, so the attack creates no combat-time nodes. Each wake samples the same easing function 65 milliseconds earlier than the preceding surface but owns no collision. `BossUtilityPool` now parents the radial slot and every boss lane/line surface under a dedicated absolute-Z `BossAttackPresentationRoot`; they are no longer descendants of `BossStructuralAdapter`, whose legitimate fallback-visibility suppression previously culled the complete S-04 effect tree. The attack surface emits one `core_shockwave_released` signal after starting the release cue; `CommandBossSession` converts that exact boundary into a restrained vertical impulse through the existing `CameraRig`. Generation cleanup, retry, armor transitions, body defeat, and recovery stop all charge/release presentation.[2] [3] [6]
...
| Visible release | One cyan-white leading ring expands continuously from the core to its authored radius, followed by four dim cosmetic wake echoes at 65-millisecond spacing. |
