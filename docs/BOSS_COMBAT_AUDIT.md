...
| **Exposed body** | Use normal melee and equipped weapons while dodging the visible projectile trajectories and managing reinforcements. | Red is body health. The enlarged moving shell or rocket is damage authority; legacy lane and echo graphics are presentation-only during the live-body fight. |
...
|---|---|---|
| **Armored / exposed** | **Fiduciary Shockwave** warns with an 820-pixel gold ring and then commits one 72-damage unblockable pulse before the global 0.75 hostile-output multiplier. Every recovery deploys four soldiers until eight are alive; casualties are replaced without exceeding the cap. | Any player damage source reduces armor, and every ordinary source damages the exposed body. Leave the 410-pixel radius before activation; dodge provides displacement but not immunity. |
| **Corpse** | The final attack frame freezes and darkens. | Use one fresh melee to create rubble and two 50-HP repair cells. |
