...
Every boss volley reserves its complete projectile requirement before presenting a warning. The reservation and one fixed telegraph record are canceled together if capacity is unavailable. Every promised shot now uses the ordinary-enemy shell/rocket renderer: one warm visible-core path, target badge, closing progress arc, firing pulse, damage intensity, and size-scaled stroke. Actual projectiles still commit from authored rig sockets or marker positions, preserving ballistics while making the warning origin easy to recognize. Recovery, armor-generation changes, retry, stop, body death, and Royal wreck transition release pending reservations, visible telegraphs, and in-flight boss shots. CANTOR's three-shell Rosary and CHOIR Prime's two-rocket Canon therefore cannot degrade into partial, duplicated, or untelegraphed attacks under pool pressure.[4][7]

Boss lane, line, and radial presentation is now independent of structural fallback visibility. One fixed `BossAttackPresentationRoot` renders above the boss/scenery layer and below the shared projectile telegraph layer. This corrects the hierarchy bug that left S-04's active particles, sphere, and ring logically visible but tree-culled whenever `BossStructuralAdapter` hid its unused fallback conductors. Cool rectangles and echoes are subordinate information; white-cyan rectangles are safe lanes; warm path/badge/arc combinations are projectile danger.[4][7]
...
The visual language is consistent: **yellow is armor, red is body health, white flash confirms accepted damage, warm gold/orange paths and target badges predict projectiles, cyan-blue particle convergence predicts S-04's radial release, cyan is otherwise memory or Disentangle, and white-cyan lanes are safe repositioning space**. Fixed pool capacities remain honest. A saturated partition causes no boss shot and no promised telegraph, ordinary projectile reuse always returns to one-to-one scale, and support caps prevent continuous spawning from becoming geometric comedy.[4][5][7]
