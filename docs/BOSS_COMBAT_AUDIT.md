...
| **Destruction spectacle** | No additional player action. | The darkened final atlas frame remains visible through the fixed 2.95-second explosion-and-fireworks sequence. Its exact completion signal automatically creates rubble, scrap, repair drops, evidence, and salvage progression once. |

The active attempt suspends ordinary rear-wall and facade collision so the player can retreat and reposition to the left. A single invisible player-only wall sits just beyond the boss's right flank and prevents the robot from crossing the live machine. It disappears as soon as body health reaches zero. Body defeat then triggers the pooled explosion-and-fireworks spectacle; the end of that sequence, not another attack, is the sole rubble and progression boundary.[1][12][13]
