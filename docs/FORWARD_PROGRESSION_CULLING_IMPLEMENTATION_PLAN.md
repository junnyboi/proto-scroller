...
| 8 — Boss/shop/corridor handoff | Complete | `cd39803` / WebDev `b5bc307b` | Each fifth facade triggers its boss first; the defeated-boss overlap opens the shop; future content remains suppressed through two road-only chunks; old facades must be culled before the next district and act activate atomically. The checkpoint also preserves concurrent district weather and progressive facade hollowing. |

## Delivered Runtime

The delivered staged-handoff revision is `cd39803`. WebDev checkpoint `b5bc307b` maps `/manus-storage/game_a4a1d66a.wasm` (39,514,754 bytes) and `/manus-storage/game_ee0f11aa.pck` (16,132,444 bytes). The checkpoint semantically preserves concurrent district weather, progressive facade hollowing, the two-stage transformer, size-aware enemy telegraphs, responsive fullscreen rendering, the managed leaderboard, and prior audio/title behavior. Full release-gate certification was intentionally not executed under the project-level user override; the required Godot 4.7.2 Web export completed successfully from shared `main`.
