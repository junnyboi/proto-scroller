...
| Progressive sprite hollowing | In progress | Pending | Replace overlay-only cavity darkening with a shader on each authored cell sprite. Normalized damage must expand one stable center-out jagged alpha void and darken surviving pixels continuously; terminal failure must erase most of the center/lower middle while retaining dark irregular side rails and a top lintel. Preserve pooling, persistence, cracks, attachments, rubble, bursts, debris, collision, and all 25 facade variants. |
