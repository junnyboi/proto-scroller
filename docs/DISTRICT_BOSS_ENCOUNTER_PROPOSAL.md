...
The strongest production strategy is to preserve the tested `CommandBossSession` as a compatibility façade and run all five encounters through one data-configured, prewarmed boss framework. One existing command tank remains the hidden authoritative combat host, while a reusable multipart rig, bounded behavior controller, arena adapter, fixed utility pool, and campaign coordinator provide unique silhouettes and mechanics.[2] [3] This approach retains the current 330-armor, 320-health, three-hit charged `jab_cross`, pooled wreck, and fixed destruction spectacle while avoiding five competing boss architectures. Spectacle completion now converts every defeated boss to rubble and advances progression without another player attack.
...
Armor failure opens the command body but does not introduce another attack symbol or cadence. The same charge-and-release cycle repeats throughout the fight, leaving support pressure and recovery opportunities—not warning ornamentation—to carry escalation. At body defeat, the universal right-flank wall drops and a restrained camera reveal frames the newly open route until the player begins moving; normal follow then resumes. The archive bracket, optional foundation cascade, bounded support cap, automatic post-spectacle rubble, and evidence outcome remain intact. The exact charge and release system is specified in [`SETTLEMENT_ENGINE_SHOCKWAVE_DESIGN.md`](SETTLEMENT_ENGINE_SHOCKWAVE_DESIGN.md).
...
![CANTOR-31 / PALE ENGINE concept](concepts/district-bosses/04-military-cantor31-pale-engine.jpg)
...
At zero health, CHOIR Prime enters **The Last Consent**, an evidence-resolved destruction phase. The immutable eligibility snapshot chooses the strongest valid result automatically when the spectacle ends:

| Outcome | Automatic eligibility | Canon result |
|---|---|---|
| **PURGE** | Selected when the snapshot lacks twenty dossiers or any one of the five evidence flags. | The weapon and coherent victim echoes are destroyed. Evidence is transmitted. “NO HOSTILES DETECTED. NO VOICES DETECTED.” |
| **DISENTANGLE** | Selected when the snapshot contains at least twenty dossiers and all five evidence flags. The five severance steps resolve immediately before rubble commit. | Military command authority is destroyed while a silent, non-weaponized archive remains. “ARCHIVE STABLE. CONSENT UNKNOWN. AWAITING YOUR DECISION.” |
| **ASCENSION FAILURE** | Historical canon branch; no longer entered by the automatic valid-outcome policy. | CHOIR uses P-01 as an exit path and leaves the city through PROTOS while multiple voices speak in unison. |

The former receiver areas remain prewarmed for save/runtime compatibility but stay inactive and invisible. No player, autonomous, or stale lethal-chain damage can delay or alter the automatic outcome. The 2.95-second destruction spectacle is the complete decision window; its one completion signal commits the eligible outcome, road-grounded rubble, repair drops, and next-step progression exactly once.
