...
The shipped opening encounter is deliberately more accessible than the later shared-connection grammar: every accepted player damage type chips S-04 armor by its actual damage. **Assessment Levy** previews and releases one traveling ground shockwave; **Double-Entry Rupture** previews and releases two. Each ghost ring corresponds to one released front, so the player can read the exact cadence before committing movement or dodge recovery. Four pooled foot soldiers arrive during recoveries until eight are active.

Armor failure opens the command body and adds **Compound Default**, a three-front red-white terminal cadence with the widest range. Below one-third body health, the triple wave leads the alternating sequence. The archive bracket, optional foundation cascade, bounded support cap, corpse finisher, and evidence outcome remain intact; only the old static and unrelated footprint attacks have been replaced. The exact telegraph and release system is specified in [`SETTLEMENT_ENGINE_SHOCKWAVE_DESIGN.md`](SETTLEMENT_ENGINE_SHOCKWAVE_DESIGN.md).
