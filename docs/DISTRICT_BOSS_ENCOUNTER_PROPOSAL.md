...
The encounter’s first phase teaches the shared boss grammar. Automatic fire efficiently strips glass, ground smash opens damaged concrete, and only a fully committed charged `jab_cross` can sever the currently highlighted 110-point steel pin. Three valid connections remove the standard 330 armor. **Settlement Sweep** and **Double-Entry Barrage** preserve one body-plus-dash safe answer. One Bulwark and one Sapper may be acquired once; the Sapper can restore concrete cover but never pins, armor, dossiers, or evidence.

Armor failure opens the 320-health command body and begins **Foreclosure Stamp**. A marked lower steel cell can drop one authored treasury slab, cancel the stamp, and apply capped bonus damage. If the cell was already destroyed, a prewarmed floor bracket appears at the same normalized anchor and direct body damage remains sufficient. The final phase introduces **Audit Beam** and one optional foundation cascade. No support actors replenish.
