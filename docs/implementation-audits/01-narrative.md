...
11. **Reveal slot reuse:** traverse beyond all four boundaries, breach configured cells, and assert fixed reveal node/slot IDs, zero post-warm creation, correct variant after reassignment, and preserved mutation state in the ledger after the damaged building is culled.
12. **No collision coupling:** revealing/collecting a dossier never changes cell health, collision layers, or `ground_passage_open()`; progression opens only after all three lower facade cells are destroyed, without a required dossier/evidence state.
