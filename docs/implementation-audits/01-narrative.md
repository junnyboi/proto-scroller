...
**Audit scope:** Project CHOIR narrative delivery, 25 dossiers, retry continuity, short transmissions, and campaign persistence.
...
The current game already exposes nearly all low-level events required by a lightweight `NarrativeDirector`, but it has **no campaign-lifetime owner, no narrative save, and no run-start/run-finish signals**. The safest implementation is an observer layer rather than a combat owner: a Main-owned, versioned `CampaignProgressStore` survives `CitySlice` replacement; a per-city `NarrativeDirector` subscribes to spatial district, siege, destruction, enemy, boss, and lifecycle events; the fixed HUD toast renders transmissions without pausing play. Facade ruins remain exclusively owned by the destruction renderer and receive no narrative background children.
...
Expose presentation requests as signals, for example `transmission_requested(event_id, speaker_key, line_key, seconds)`, `dossier_collected(definition, total, district_total)`, and `continuity_updated(generation)`. `CitySlice` wires these to the HUD. The director must never pause `UrbanSiegeRuntime`, acquire a `RunPauseCoordinator` token, modify collision, award score, release enemies, create facade visuals, or own combat spawn caps.
...
### 4. Keep facade presentation outside the narrative layer

The stream guarantees six reusable buildings (`CityWorldStream.CHUNK_CAPACITY`) and tests assert zero post-warm node creation. Dossier collection may observe a destroyed cell, but it must not add sprites, textures, plates, or cross-section nodes to any resident building. The destruction system's authored facade erosion, jagged arch, rubble sprites, and pooled smoke are the complete visual result across all twenty-five variants.
...
| Add | `game/resources/narrative/dossiers/*.tres` and narrative art | 25 dossier definitions and codex images. |
| Modify | `game/scripts/main/main.gd` | Own/load store; inject it into title and each new city; preserve it across retry. |
| Modify | `game/scripts/gameplay/city_slice.gd` | Accept store, create the director, and wire transmission signals. |
...
| Adding narrative backgrounds on destruction | Obscures procedural ruins and violates fixed node-growth guarantees. | Keep facade presentation exclusively in the destruction renderer; assert no narrative child nodes across all 25 variants. |
...
1. **Catalog bijection:** `DossierCatalog.validation_errors()` is empty; exactly 25 dossier IDs map one-to-one to `CityDistrictCatalog` variants, exactly five per district, and all trigger cells are in the 3×2 bounds.
...
9. **Spatial transition semantics:** clear each five-facade district, move to chunks 5/10/15/20, and assert four arrival events; the 500-pixel rear frontier prevents returning to a prior district. Separately emit `UrbanSiegeRuntime.district_completed` and assert it does not mark a spatial district truth complete.
...
11. **No cross-section backgrounds:** apply all 25 variants, collect representative dossiers, and assert resident building child counts stay fixed with no narrative image or cross-section node before or after streaming reassignment.
12. **No collision coupling:** collecting a dossier never changes cell health, collision layers, or `ground_passage_open()`; progression opens only after all three lower facade cells are destroyed, without a required dossier/evidence state.
...
15. **Runtime budget:** extend `test_runtime_budget.gd` to assert the fixed narrative director and toast counts; after 100 transmission requests and three retries, node count and capacities remain at baseline with no post-warm allocation.
...
Implement the Main-owned store and catalog validation first, then the observer director and explicit finish signal, and finally the nonblocking HUD toast. This order proves retry persistence and event correctness without coupling narrative state to facade rendering. Phase one should ship Business and Residential definitions through the same 25-entry validated catalog shape (with unavailable entries rejected in release validation or filled before release), so later districts add content rather than alter persistence semantics.
