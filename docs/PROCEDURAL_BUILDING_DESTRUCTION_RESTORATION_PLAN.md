...
Terminal rubble no longer stretches the same authored strip across a full cell or anchors upper-floor collapse to street level. Each ground bay derives a stable presentation from its facade seed and structural material and reuses only transparent concrete, glass, or steel fragment sprites. Every structural pile settles on the road below its destroyed ground section; upper-floor terminal arches retain no floating rubble. These shallow foreground fragments are persistent rubble, not a replacement facade, cross-section, interior plate, or cell-sized background.
...
## WP9 — Rubble-only terminal policy across destructible structures

The current policy makes persistent rubble explicit without reviving the removed backgrounds. The three ground bays of each streamed district facade prewarm one four-fragment material-matched bed and reveal it only at terminal failure; upper bays keep their jagged facade shell without a fragment bed. Streamed cars and streetlamps and pooled catalyst structures prewarm three-fragment steel beds; their authored wreck or spent texture remains visible only during the nonterminal damaged stage and is hidden at terminal destruction. The existing boss rubble presentation remains unchanged. Enemy remains and temporary hazard actors retain their separate bounded lifecycles.

`PersistentRubbleBed2D` is visual-only and accepts only the three transparent material-fragment textures already used by debris. It has no collision object, facade texture, shader sample, rectangular fill, or runtime-spawn path. Structural visibility derives from the cell's restored destroyed state; prop and catalyst visibility derives from `is_fully_destroyed`. Pristine reset and pooled reconfiguration hide the bed deterministically, while physical debris and section bursts remain transient and do not carry persistence responsibility.
