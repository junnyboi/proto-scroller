...
Terminal rubble no longer stretches the same authored strip across every full cell or anchors upper-floor collapse to street level. Each cell derives a stable presentation from its facade seed and structural material and reuses only transparent concrete, glass, or steel fragment sprites. Every pile settles at the bottom of its own destroyed section, so upper-floor material rests on the floor beneath the cavity while ground-row material remains street-supported. These shallow foreground fragments are persistent rubble, not a replacement facade, cross-section, interior plate, or cell-sized background.
...
The current policy makes persistent rubble explicit without reviving the removed backgrounds. All six cells of every one of the 25 district facades prewarm a four-fragment material-matched bed and reveal it only at terminal failure. Streamed cars and streetlamps and pooled catalyst structures prewarm three-fragment steel beds; their authored wreck or spent texture remains visible only during the nonterminal damaged stage and is hidden at terminal destruction. The existing boss rubble presentation remains unchanged. Enemy remains and temporary hazard actors retain their separate bounded lifecycles.
