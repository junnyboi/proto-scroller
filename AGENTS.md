...
Do not pause to ask the user which conflict side or fast-forwarded feature to keep. Always take the recommended merge-both path: retain the active requested change, preserve compatible upstream behavior, resolve the combined contract directly, and continue through the lightweight regression, push, export, checkpoint, and deployment sequence.
