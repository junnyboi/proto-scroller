...
The Web host chooses the matching orientation through media queries. The static picture is visible immediately, then the silent autoplaying video fades in after its first playable frame. Godot keeps the title viewport transparent on Web only. When the expedition begins, Godot disables transparency and calls the host bridge to pause and hide the video. Native builds continue to use the canonical static artwork.
